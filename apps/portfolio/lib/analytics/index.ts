...
export const events = ["$pageview", "project_link_clicked", "contact_link_clicked", "resume_clicked", "ask_opened", "ask_question_submitted", "ask_response_completed", "ask_failed"] as const;
...
export function track(event: AnalyticsEvent, properties: EventProperties = {}) {
  try {
    ...
    client?.capture(event, { ...allowProperties(properties), app: "portfolio", environment: process.env.NEXT_PUBLIC_POSTHOG_ENVIRONMENT, pathname: safePath(pathname) });
    return true;
    ...
    if (!client) {
      ...
      client ??= posthog.init(process.env.NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN!, {
        ...
        before_send: (event) => {
          ...
          const safe: Record<string, unknown> = { $ip: "0.0.0.0", $geoip_disable: true, token: process.env.NEXT_PUBLIC_POSTHOG_PROJECT_TOKEN, ...allowProperties(props), app: "portfolio", environment: process.env.NEXT_PUBLIC_POSTHOG_ENVIRONMENT, pathname: safePath(pathname), $current_url: location.origin + safePath(pathname) };
