...
export default function WorkPage() { const booking = getBookingDestination(); return <main id="main-content"><WorkSection /><section id="work-contact" className="site-shell v2-page-tail"><p className="section-kicker">Your project</p><h2>Have a similar problem?</h2><p>Show me the workflow and what the first useful version needs to do.</p><div className="hero-actions"><a className="button" href={booking.href} rel={booking.configured ? "noreferrer" : undefined} target={booking.configured ? "_blank" : undefined}>{booking.configured ? "Book a call" : "Request a call"}</a><Link className="text-link" href="/contact">Send a brief</Link></div></section></main>; }
