...
export function ServiceDetailPage({ slug }: { slug: DraftServiceSlug }) {
  ...
  return <main id="main-content" className="mvp-page service-detail-page">
    <section className="mvp-hero" aria-labelledby="service-title"><div className="site-shell mvp-hero__layout">
      <div><Link className="mvp-back-link" href="/services">← All services</Link><p className="section-kicker">{service.name} / {service.number}</p><h1 id="service-title">{service.title}</h1><p className="mvp-hero__lead">{service.lead}</p><div className="hero-actions"><a className="button" href={booking.href} rel={booking.configured ? "noreferrer" : undefined} target={booking.configured ? "_blank" : undefined}>{booking.configured ? "Book a call" : "Request a call"}</a><Link className="text-link" href="/contact">Send a brief →</Link></div></div>
    ...
    <section className="mvp-close" aria-labelledby="service-close-title"><div className="site-shell"><p className="section-kicker">Start a conversation</p><h2 id="service-close-title">{service.closeTitle}</h2><p>{service.close}</p><div className="hero-actions"><a className="button button--light" href={booking.href} rel={booking.configured ? "noreferrer" : undefined} target={booking.configured ? "_blank" : undefined}>{booking.configured ? "Book a call" : "Request a call"}</a><Link className="text-link" href="/contact">Send a brief →</Link></div></div></section>
  </main>;
}
