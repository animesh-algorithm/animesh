...
The `animesh.cc` DNS Domain property was inspected on 2026-09-18. Search Console reports Animesh Sharma as a verified owner and says the property was added that day. At that inspection, performance, indexing, crawl and Core Web Vitals reports were still processing, and Portfolio and Hire had not yet deployed their new `robots.txt` routes. The September 25 audit below records the later live and Search Console state.
...
All three sitemaps are live and were resubmitted in Search Console on September 25:
...
- Later on September 25, each sitemap still showed `Couldn't fetch` and zero discovered pages; the Portfolio sitemap detail showed a September 25 read attempt. Google Search Console's live URL inspection of all three sitemap URLs reported `Crawl allowed? Yes` and `Page fetch Successful` as the Google Inspection Tool smartphone. This proves those live tests could fetch the URLs, not that sitemap processing succeeded or that normal Googlebot fetches always succeed. The Domain property's Manual actions report showed `No issues detected`. Recheck the sitemap report after Google has had time to process the submissions; if the failure persists, investigate actual crawl responses or hosting logs before changing sitemap generation.
